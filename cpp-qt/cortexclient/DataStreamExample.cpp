...
DataStreamExample::DataStreamExample(QObject *parent) : QObject(parent),
    ...
    QFile inputFile("OSC.txt");
    if (inputFile.open(QIODevice::ReadOnly))
    {
       QTextStream in(&inputFile);
       QString address = in.readLine();
       qint16 port = in.readLine().toShort();
       qInfo() << "Sending to " << address << " " << port;
       _oscSender->setRemoteHost(address);
       _oscSender->setRemotePort(port);
       _oscSender->setup();
       inputFile.close();
    }
}
...
void DataStreamExample::onStreamDataReceived(
    ...
    qInfo() << data;
    QOSCMessage *message = new QOSCMessage("/eeg");

    for(int i=0; i< data.count(); ++i) {
        float value = data.at(i).toDouble();
        //qInfo() << i << " " << value;
